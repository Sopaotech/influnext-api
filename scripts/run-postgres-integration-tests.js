...
try {
  ...
  run(process.execPath, [
    ...
    '--runInBand',
    '--runTestsByPath',
    path.join('tests', 'integration', 'postgres.integration.test.ts'),
  ], {
    ...process.env,
  ...
  console.error('[integration-test-harness] blocked:', error instanceof Error ? error.message : 'unknown error');
  process.exit(1);
}
