  ...
  if (isLoading) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center space-y-4">
        <Loader2 className={`w-8 h-8 ${isCompany ? 'text-amber-500 animate-spin' : 'text-purple-500 animate-spin'}`} />
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2 mb-2">
               <div className={`h-1.5 w-10 ${isCompany ? 'bg-pink-600' : 'bg-purple-600'} rounded-full`} />
               <span className={`text-[10px] font-black ${isCompany ? 'text-pink-500' : 'text-purple-500'} uppercase tracking-[0.4em]`}>
            ...
            <p className={`text-[11px] font-black max-w-lg uppercase tracking-widest px-3 py-1.5 rounded-lg w-fit border ${
              isDark 
                ? (isCompany ? 'bg-pink-950/30 border-pink-500/10 text-pink-300' : 'bg-purple-950/30 border-purple-500/10 text-purple-300') 
                : (isCompany ? 'bg-pink-500/10 border-pink-500/20 text-pink-700' : 'bg-purple-500/10 border-purple-500/20 text-purple-700')
            }`}>
          ...
          <div className="space-y-6">
            ...
            <div className={`border p-6 rounded-[2rem] space-y-4 border-l-4 border-l-pink-500 shadow-sm relative overflow-hidden ${
              ...
              <Button 
                ...
                className="w-full h-11 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white font-black text-[9px] uppercase tracking-widest rounded-xl transition-all shadow-sm"
              >
                  ...
                  {user?.subscriptionTier === 'FREE' && (
                    <div className="px-4 py-2.5 bg-purple-950/30 border border-purple-500/10 rounded-2xl mb-2 flex items-center justify-between animate-in slide-in-from-bottom-2 duration-500">
                      <span className="text-[10px] font-black text-purple-300 uppercase tracking-widest flex items-center gap-2">
                        <Crown className="w-3 h-3 text-purple-400" /> Mentor IA de Carreira Limitado no Plano Gratuito
                      </span>
                      <Button 
                        variant="link" 
                        onClick={() => router.push('/dashboard/subscription')}
