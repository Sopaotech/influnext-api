    ...
    <div className="p-4 md:p-10 max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-1000 pb-32">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-10 border-b border-white/[0.08]">
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-purple-400 font-black text-[10px] tracking-[0.4em] uppercase">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                 <div className="px-3 py-1 bg-white/5 rounded-lg text-[10px] font-black text-zinc-400 uppercase tracking-widest border border-white/10">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={handleShare}
            className="px-6 py-4 bg-white/5 border border-white/10 hover:border-purple-300 hover:bg-white/10 rounded-2xl flex items-center gap-3 transition-all font-bold text-sm text-white shadow-sm"
          >
            <Share2 className="w-4 h-4" /> Compartilhar Kit
          </button>
          <button className="px-8 py-4 bg-white/10 hover:bg-purple-600 text-white rounded-2xl flex items-center gap-3 transition-all font-black text-[10px] uppercase tracking-widest shadow-xl">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-black/35 backdrop-blur-md border border-white/5 rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 group">
               ...
               <div className="space-y-1">
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Taxa de Engajamento</p>
                  <h3 className="text-5xl font-black text-white tracking-tighter drop-shadow-sm">{kpis?.latestEngagement || '0.0'}%</h3>
               </div>
               <div className="mt-4 flex items-center gap-2 text-zinc-400 font-black text-[10px] uppercase tracking-widest bg-white/5 w-fit px-3 py-1 rounded-full border border-white/10">
                  Média do nicho: 2.1%
               </div>
            ...
            <div className="bg-black/35 backdrop-blur-md border border-white/5 rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 group">
               ...
               <div className="space-y-1">
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Views Médias / Post</p>
                  <h3 className="text-5xl font-black text-white tracking-tighter drop-shadow-sm">{kpis?.avgViews?.toLocaleString() || '0'}</h3>
               </div>
            </div>
         </div>
      </section>
      ...
      <section className="bg-black/35 backdrop-blur-xl border border-white/5 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden">
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6 relative z-10">
            <div className="space-y-1">
               ...
               <h2 className="text-4xl font-black text-white tracking-tighter">Catálogo de Serviços</h2>
            </div>
            <div className="bg-white/10 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 relative z-10">
            {data?.rateCard?.length > 0 ? data.rateCard.map((rate: any, idx: number) => (
               <div key={idx} className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[2rem] hover:scale-[1.02] transition-all duration-500 group shadow-sm hover:shadow-xl">
                  <div className="flex justify-between items-start mb-4 md:mb-6">
                     <div className="p-3 bg-white/10 text-white rounded-xl group-hover:bg-purple-600 transition-colors">
                  ...
                  <h4 className="text-base md:text-lg font-black text-white uppercase tracking-tight mb-2">{rate.serviceName}</h4>
                  <p className="text-[11px] md:text-xs text-zinc-400 font-medium leading-relaxed">{rate.description}</p>
               </div>
            )) : (
      ...
      <section className="bg-black/35 backdrop-blur-xl border border-white/5 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-sm relative z-10">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 md:mb-12 gap-4">
           <div className="space-y-1">
              <div className="flex items-center gap-2 text-indigo-400 font-black text-[10px] md:text-[11px] tracking-[0.4em] uppercase">
                <PieChart className="w-4 h-4" /> Demografia_Verified
              </div>
              <h2 className="text-2xl md:text-3xl font-black text-white tracking-tighter">Análise de Audiência</h2>
           </div>
           <div className="flex items-center gap-2 text-[9px] md:text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] bg-white/5 px-4 py-2 rounded-full border border-white/10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20">
           <div className="space-y-8">
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 border-l-2 border-purple-500 pl-3">Principais Cidades</h4>
              <div className="space-y-6">
                 {[
                   ...
                   { name: 'Curitiba', p: 9, color: 'bg-zinc-700' }
                 ].map(city => (
                   <div key={city.name} className="space-y-2">
                      <div className="flex justify-between items-end">
                         <span className="text-xs md:text-sm font-bold text-zinc-300">{city.name}</span>
                         <span className="text-[10px] md:text-xs font-black text-white">{city.p}%</span>
                      </div>
           ...
           <div className="space-y-8">
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 border-l-2 border-purple-500 pl-3">Faixa Etária & Gênero</h4>
              <div className="flex flex-col sm:flex-row items-center gap-10 md:gap-10">
                 <div className="relative w-32 h-32 md:w-40 md:h-40">
                    <svg viewBox="0 0 36 36" className="w-full h-full rotate-[-90deg]">
                       <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="6" />
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                       <span className="text-xl md:text-2xl font-black text-white">65%</span>
                    ...
                    {[
                      ...
                      <div key={age.age} className="flex items-center gap-4">
                         ...
                         <div className="flex-1 h-1 md:h-1.5 bg-white/10 rounded-full overflow-hidden">
                            <div className="h-full bg-white rounded-full" style={{ width: `${age.p}%` }} />
                         </div>
                         <span className="text-[9px] md:text-[10px] font-black text-white w-8">{age.p}%</span>
                      </div>
                    ))}
                 </div>
              </div>
