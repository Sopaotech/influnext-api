...
import { FileText, ShieldCheck, Clock, CheckCircle2, AlertCircle, ExternalLink, Zap, ChevronDown, ChevronUp, Brain, Sparkles, Loader2, Send } from 'lucide-react';
...
export default function ContractsPage() {
  ...
  useEffect(() => {
    fetchContracts();
    setUserRole(Cookies.get('influnext_role') || null);
  }, []);
  ...
  const handleSaveScript = async (contractId: string) => {
    try {
      ...
      toast.success('✦ Roteiro/Opinião atualizado com sucesso!', {
        description: 'Sua orientação/opinião foi salva e está vinculada ao contrato.'
      });
      setEditingScriptId(null);
      fetchContracts();
    } catch (err: any) {
      toast.error(err.response?.data?.error || 'Erro ao salvar roteiro.');
    } finally {
  ...
  const handleSubmitDeliverable = async (deliverableId: string) => {
    const proofUrl = proofUrls[deliverableId];
    if (!proofUrl) {
      toast.error('Por favor, informe a URL da prova de entrega.');
      return;
    }

    try {
      setSubmittingIds(prev => ({ ...prev, [deliverableId]: true }));
      const res = await api.patch(`/deliverables/${deliverableId}/submit`, { proofUrl });
      toast.success(res.data.message || 'Entrega enviada com sucesso!');

      const audit = res.data.aiAudit;
      if (audit) {
        if (audit.approved) {
          toast.success(`✦ Auditoria de IA: Aprovada! (Confiança: ${audit.confidenceScore}%)`, {
            description: audit.feedback,
            duration: 8000
          });
        } else {
          toast.warning(`⚠ Auditoria de IA: Atenção necessária! (Confiança: ${audit.confidenceScore}%)`, {
            description: audit.feedback,
            duration: 8000
          });
        }
      ...
      const errorMsg = err.response?.data?.error || 'Erro ao enviar entregável.';
                              ...
                              <div className="space-y-4">
                                 ...
                                 <div className="space-y-4">
                                    {!contract.deliverables || contract.deliverables.length === 0 ? (
                                      <p className="text-zinc-600 text-xs italic">Nenhum entregável cadastrado para este contrato.</p>
                                    ) : (
                                      contract.deliverables.map((d) => {
                                        ...
                                        let statusBadgeColor = 'text-zinc-400 bg-zinc-400/10 border-zinc-400/20';
                                        if (isDone) statusBadgeColor = 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20';
                                        if (isReview) statusBadgeColor = 'text-blue-400 bg-blue-400/10 border-blue-400/20';

                                        return (
                                          <div key={d.id} className="p-5 bg-[#100c1e] border border-[#1e1430] rounded-2xl space-y-3 shadow-md">
                                            <div className="flex justify-between items-start gap-2">
                                              <div>
                                                <h4 className="text-xs font-black text-zinc-200 uppercase tracking-wider">{d.title}</h4>
                                                <span className="text-[8px] bg-purple-500/15 text-purple-400 px-2 py-0.5 rounded border border-purple-500/10 font-bold uppercase tracking-widest">{d.type}</span>
                                              </div>
                                            ...
                                            {isReview && d.proofUrl && (
                                              <div className="pt-2 border-t border-zinc-800/40">
                                                <a 
                                                  ...
                                                  target="_blank" 
                                                  rel="noreferrer" 
                                                  className="text-[9px] text-blue-400 hover:underline flex items-center gap-1 font-bold uppercase tracking-widest"
                                                >
                                            ...
                                            {isDone && d.proofUrl && (
                                              <div className="pt-2 border-t border-zinc-800/40">
                                                <a 
                                                  ...
                                                  target="_blank" 
                                                  rel="noreferrer" 
                                                  className="text-[9px] text-emerald-400 hover:underline flex items-center gap-1 font-bold uppercase tracking-widest"
                                                >
                                              ...
                                              <div className="space-y-2 pt-2 border-t border-zinc-800/40">
                                                <div className="flex gap-2">
                                                  <input 
                                                    ...
                                                    className="flex-1 bg-black/45 border border-[#1e1430] rounded-xl px-3 py-2 text-xs text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-purple-500/50 transition-colors"
                                                  />
                                                  <button 
                                                    onClick={() => handleSubmitDeliverable(d.id)}
                                                    disabled={submittingIds[d.id]}
                                                    className="bg-purple-600 hover:bg-purple-500 disabled:opacity-40 disabled:hover:bg-purple-600 text-white p-2.5 rounded-xl transition-all shadow-md active:scale-95 flex items-center justify-center"
                                                  >
                                                    {submittingIds[d.id] ? (
