...
import { TrendingUp, ArrowUpRight, DollarSign, Activity, FileText, Download } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  // Dados Mockados para o Gráfico de Exemplo
  const monthlyData = [
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-10 pb-24 animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div className="bg-white/40 border border-white/50 p-6 rounded-[2rem] shadow-xl" style={{ backdropFilter: 'blur(30px)' }}>
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-1">Faturamento Anual (Est.)</p>
            <p className="text-4xl font-black text-slate-900 tracking-tighter">
              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(58000 + balance)}
            </p>
         </div>
         ...
         <div className="bg-white/40 border border-white/50 p-6 rounded-[2rem] shadow-xl" style={{ backdropFilter: 'blur(30px)' }}>
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-1">Taxa de Engajamento</p>
            <p className="text-4xl font-black text-slate-900 tracking-tighter">
              6.8%
            </p>
         </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-8 md:p-10 shadow-2xl relative overflow-hidden group">
            ...
            {monthlyData.map((data, idx) => {
              ...
              return (
                <div key={idx} className="flex-1 flex flex-col items-center justify-end gap-3 group/bar">
                  {/* Tooltip escondido */}
                  <div className="opacity-0 group-hover/bar:opacity-100 transition-opacity bg-white text-slate-900 text-xs font-black px-3 py-2 rounded-xl mb-2 whitespace-nowrap shadow-xl translate-y-2 group-hover/bar:translate-y-0 duration-300">
      ...
      <div className="bg-white/60 border border-white/50 rounded-[3rem] p-8 md:p-10 shadow-xl" style={{ backdropFilter: 'blur(30px)' }}>
        <div className="flex items-center gap-3 mb-8">
           <div className="w-10 h-10 rounded-2xl bg-slate-900 text-white flex items-center justify-center shadow-lg">
              <FileText size={18} />
           </div>
           <div>
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Extrato</h3>
