    ...
    <div className="p-4 md:p-12 max-w-[1600px] mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-24">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-10 border-b border-white/[0.08]">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-violet-400 font-black text-[10px] tracking-[0.4em] uppercase">
            <ShieldCheck className="w-5 h-5 text-violet-400" />
            GovernanceHub // FounderLevel
          </div>
          <h1 className="text-5xl font-black text-white tracking-tighter">
            Análise de <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-500 to-pink-500">Mercado</span>
          </h1>
        </div>
      ...
      <section className="bg-white/[0.02] border border-white/[0.06] rounded-[2.5rem] p-8 space-y-6 shadow-2xl backdrop-blur-xl relative overflow-hidden">
        {/* Glow corner */}
        <div className="absolute top-0 right-0 w-32 h-32 bg-violet-600/5 blur-3xl rounded-full" />
        
        <div className="flex items-center justify-between">
           <div className="space-y-1">
              <div className="flex items-center gap-2 text-violet-400 font-black text-[10px] tracking-[0.2em] uppercase">
                <Crown className="w-4 h-4 text-violet-400" />
                Acesso Pro & Parcerias
              </div>
              <h2 className="text-xl font-black text-white tracking-tight">Liberar Vantagens para Parceiros</h2>
           </div>
        </div>
        
        <div className="flex flex-col md:flex-row gap-4 p-6 bg-white/[0.02] rounded-3xl border border-white/[0.06]">
           <div className="flex-1 relative">
              ...
              <input 
                ...
                className="w-full bg-white/[0.04] border border-white/[0.08] text-white rounded-2xl pl-11 pr-4 py-4 text-sm font-bold focus:outline-none focus:border-violet-500/50 focus:bg-white/[0.07] transition-all placeholder:text-zinc-600 shadow-sm"
           ...
           <button 
             ...
             disabled={isGranting}
             className="bg-white hover:bg-zinc-200 text-slate-950 font-black text-[10px] uppercase tracking-[0.2em] px-10 rounded-2xl h-14 transition-all shadow-xl active:scale-[0.98] disabled:opacity-50"
           >
             {isGranting ? 'Sincronizando...' : 'Liberar Acesso Pro →'}
           </button>
        </div>
        <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest text-center">Isso ativará todos os recursos premium e removerá as travas de pagamento do usuário.</p>
      </section>

      {/* Social Assets Section for Admin */}
      <section className="bg-gradient-to-r from-violet-950/20 via-pink-950/10 to-violet-950/20 border border-violet-500/20 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
         ...
         <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="space-y-4">
               <div>
                  <div className="text-violet-400 font-black text-[10px] tracking-[0.3em] uppercase mb-1">Configuração de Elite</div>
            ...
            <div className="flex flex-wrap gap-4">
               <button 
                 ...
                 className="px-6 py-4 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] rounded-2xl flex items-center gap-3 transition-all group"
               >
      ...
      <section className="bg-white/[0.02] border border-white/[0.06] rounded-[2.5rem] p-8 relative overflow-hidden shadow-2xl backdrop-blur-xl">
         <div className="absolute top-0 right-0 p-8 opacity-5">
            <Zap className="w-48 h-48 text-violet-600" />
         </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
               <div>
                  <div className="flex items-center gap-2 text-pink-400 font-black text-[10px] tracking-[0.3em] uppercase mb-1">
                    <Sparkles className="w-4 h-4 text-pink-400 animate-pulse" /> Gemini Growth Partner
                  </div>
                  <h2 className="text-2xl font-black text-white tracking-tight">Estrategista de Escala SaaS</h2>
               </div>
               <button 
                 onClick={handleGenerateStrategy}
                 disabled={isGenerating}
                 className="px-6 py-3 bg-violet-600 hover:bg-violet-500 text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition-all active:scale-95 shadow-lg shadow-violet-600/20 disabled:opacity-50"
               >
                 {isGenerating ? 'Analisando Mercado...' : 'Gerar Novo Plano Estratégico'}
               </button>
            </div>
            {strategy ? (
              <div className="bg-white/[0.01] border border-white/[0.05] rounded-2xl p-6 md:p-10 max-w-none animate-in fade-in zoom-in-95 duration-500 shadow-inner">
                 <div className="text-zinc-300 text-sm leading-relaxed whitespace-pre-wrap font-sans">
                    {strategy.content}
                 </div>
              </div>
            ) : (
              <div className="h-48 border-2 border-dashed border-white/[0.08] rounded-2xl flex flex-col items-center justify-center space-y-4 bg-white/[0.01]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Support Tickets */}
        <div className="lg:col-span-2 bg-white/[0.02] border border-white/[0.06] rounded-[2.5rem] p-6 shadow-sm space-y-6 backdrop-blur-xl">
           <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2">
              <LifeBuoy className="w-3.5 h-3.5 text-zinc-400" /> Chamados de Suporte
           </h3>
           <div className="space-y-3">
              {tickets.length > 0 ? tickets.map((t: any) => (
                <div key={t.id} className="p-4 bg-white/[0.02] border border-white/[0.06] rounded-2xl flex items-center justify-between group hover:border-violet-500/40 hover:bg-white/[0.04] transition-all">
                   <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-zinc-400 font-medium truncate max-w-[300px]">{t.message}</p>
                   </div>
                   <div className="text-right">
                      <p className="text-[9px] font-black text-zinc-500 uppercase">{t.user.email}</p>
                      <button className="text-[9px] font-black text-violet-400 uppercase tracking-widest hover:text-violet-300">Responder</button>
                   </div>
                </div>
              )) : (
        ...
        {/* User Breakdown */}
        <div className="bg-white/[0.02] border border-white/[0.06] rounded-[2.5rem] p-6 shadow-sm space-y-6 backdrop-blur-xl">
           <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Base de Usuários</h3>
           <div className="space-y-3">
              {Array.isArray(data?.metrics?.totalUsers) ? data.metrics.totalUsers.map((u, idx) => (
                <div key={idx} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/[0.06] rounded-2xl">
                    <span className="text-[10px] font-black uppercase text-zinc-400">{u.role}</span>
                    <span className="text-xl font-black text-white">{u._count._all}</span>
                </div>
              )) : null}
           </div>
        </div>
