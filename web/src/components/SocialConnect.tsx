    ...
    return (
      <div 
        ...
        className={`min-w-[180px] p-5 rounded-2xl bg-white border transition-all group flex flex-col items-center gap-4 text-center ${
          acc.connected ? 'border-orange-200 bg-orange-50/30' : 'border-slate-100 ' + details.border
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className={`text-[10px] font-medium tracking-wide ${acc.connected ? 'text-orange-600' : 'text-slate-400'}`}>
        ...
        <button 
          ...
          acc.connected 
          ? 'bg-white text-orange-600 border border-orange-100 shadow-sm cursor-default' 
          : 'bg-slate-900 text-white hover:bg-orange-600 hover:shadow-lg hover:shadow-orange-200'
        }`}>
          {acc.connected ? (
            <span className="flex items-center justify-center gap-2">Vinculado <CheckCircle2 className="w-3.5 h-3.5 text-orange-500" /></span>
          ) : (
  ...
  return (
    <section className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-sm space-y-8">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
           <h3 className="text-xs font-black uppercase tracking-[0.2em] text-orange-600">Redes Sociais</h3>
