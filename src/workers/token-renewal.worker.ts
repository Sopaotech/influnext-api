...
export const runTokenRenewalLogic = async () => {
  ...
  // 2. Instagram Basic Display: Tokens expiram em 60 dias.
  // Vamos buscar tokens ativos do Instagram que expiram em menos de 15 dias.
  ...
  for (const platform of instagramPlatforms) {
    try {
      ...
      try {
        ...
        console.log(`[TOKEN_RENEWAL] ✅ Token do Instagram (Basic) renovado com sucesso para @${platform.username}.`);
      } catch (refreshErr: any) {
        // Se falhar (por ser token Business Graph API ou outro erro), enviamos a notificação preventiva
        console.warn(`[TOKEN_RENEWAL] Falha no refresh programático (provável conta Business). Disparando notificação preventiva para @${platform.username}.`);
