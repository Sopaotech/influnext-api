...
A InfluNext é dividida em três pilares principais para garantir portabilidade de código e máxima velocidade de entrega:
...
*   **Cobranças/Assinaturas**: Integração nativa com **Stripe API** e Stripe Connect (Contas Express).
...
*   **PWA**: `@ducanh2912/next-pwa` para suporte completo a Progressive Web App (instalação direta no mobile, caching offline, responsividade total).

### C. Aplicativo Mobile
*   **SDK**: **Flutter (Dart SDK ^3.12.2)**.
*   **Arquitetura**: Aplicativo híbrido do tipo **Web Shell (WebView)**.
*   **Funcionamento**: Incorpora a tela do PWA Next.js diretamente dentro de uma casca nativa usando `webview_flutter`. Em tempo de compilação, redireciona o tráfego dinamicamente para o localhost do emulador ou para a URL de produção.
...
└── mobile/                   # Aplicativo Nativo (Flutter/Dart)
    ├── lib/main.dart         # Inicialização do app Flutter
    └── lib/screens/          # Telas nativas (PwaWebViewScreen)
```
...
sequenceDiagram
    ...
    Mobile->>Web: Renderiza o PWA local (10.0.2.2:3000) ou produção
    ...
    Web-->>Mobile: Atualiza visualmente a interface de forma fluida
```
...
Para rodar todo o ambiente de desenvolvimento em sua máquina local, execute os passos abaixo. Certifique-se de ter o **Node.js (v18+)**, **Redis** e o **Flutter SDK** instalados.
...
### Passo 3: Rodar o Aplicativo Mobile (Flutter)
1. Certifique-se de que um emulador Android ou simulador iOS está rodando.
2. Navegue até a pasta `mobile`:
   ```bash
   cd ../mobile
   ```
3. Busque os pacotes de dependências:
   ```bash
   flutter pub get
   ```
4. Rode a aplicação. A WebView detectará automaticamente o ambiente de testes e carregará o Next.js local em `http://10.0.2.2:3000`:
   ```bash
   flutter run
   ```
...
## 🚀 5. Como Desenvolver Mais Recursos

### Para Adicionar uma Nova Rota ou Funcionalidade na API:
1.  **Banco de Dados**: Se a funcionalidade exigir novos dados, modifique o [schema.prisma](file:///d:/Influnext/prisma/schema.prisma) e execute `npx prisma db push` para atualizar o banco.
...
*   Construa componentes funcionais em React, estilizando-os rapidamente com **Tailwind CSS v4** e utilizando os blocos do **shadcn/ui** já configurados.
*   Conecte as telas aos endpoints que criou na API utilizando a biblioteca **Axios** instanciada nas requisições.
*   Teste no dispositivo móvel ou no emulador Flutter para garantir que o WebView se comporta exatamente como esperado e que o layout seja responsivo e premium.
