...
exports.simulateFlowStep = exports.triggerTokenRenewalDebug = exports.getConnectedPlatforms = exports.syncPlatformMetrics = exports.handleTikTokCallback = exports.simulateInstagramConnection = exports.handleInstagramCallback = exports.getAuthUrls = void 0;
...
const getAuthUrls = async (req, res) => {
    try {
        ...
        const stateBasic = jsonwebtoken_1.default.sign({ userId, from: from || '', isBusiness: false }, jwtSecret, { expiresIn: '1h' });
        const stateBusiness = jsonwebtoken_1.default.sign({ userId, from: from || '', isBusiness: true }, jwtSecret, { expiresIn: '1h' });
        const stateTiktok = jsonwebtoken_1.default.sign({ userId, from: from || '' }, jwtSecret, { expiresIn: '1h' });
        const scopes = [
            'instagram_basic',
...
const handleInstagramCallback = async (req, res) => {
    ...
    try {
        ...
        let decodedState;
        try {
            decodedState = jsonwebtoken_1.default.verify(stateStr, jwtSecret);
        }
        catch (jwtErr) {
...
const simulateFlowStep = async (req, res) => {
    try {
        ...
        const influencerUser = await prisma_1.prisma.user.findUnique({
        ...
        if (step === 1) {
            ...
            await prisma_1.prisma.$transaction([
                prisma_1.prisma.socialPlatform.upsert({
                ...
                prisma_1.prisma.metricSnapshot.create({
                    data: {
                        influencerId: influencer.id,
                        provider: 'INSTAGRAM',
                        followers,
                ...
                prisma_1.prisma.influencerProfile.update({
                    where: { id: influencer.id },
                    data: {
                        verifiedMetrics: true,
                        insights: JSON.stringify(insightsJson),
            ...
            await scoring_service_1.ScoringService.calculateAndPersist(influencer.id);
            const updatedProfile = await prisma_1.prisma.influencerProfile.findUnique({
                where: { id: influencer.id }
            });
            res.json({
                success: true,
        ...
        if (step === 2) {
            ...
            const analysis = await ai_service_1.AIService.generateWeeklyAnalysis(influencer.id);
            res.json({
                success: true,
        ...
        if (step === 4) {
            ...
            const contract = await prisma_1.prisma.contract.findFirst({
                where: {
                    companyId: company.id,
                    influencerId: influencer.id,
                    escrowStatus: 'DRAFT'
        ...
        if (step === 5) {
            ...
            const auditResult = await ai_service_1.AIService.auditDeliverableLink(proofUrl, deliverable.type, contract.title, contract.briefing || undefined, contract.aiScript || undefined);
            // Atualiza entregável para UNDER_REVIEW com a prova
            const updatedDeliverable = await prisma_1.prisma.deliverable.update({
...
exports.simulateFlowStep = simulateFlowStep;
